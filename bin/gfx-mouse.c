...
/*static uint8_t cursor[CURSOR_WIDTH * CURSOR_HEIGHT] = {
	...
	0xff, 0xee, 0xcc, 0x00, 0x00, 0x00, 0x00, 0x00,
	0xff, 0xff, 0xee, 0xcc, 0x00, 0x00, 0x00, 0x00,
	0xff, 0xff, 0xff, 0xee, 0xcc, 0x00, 0x00, 0x00,
	0xff, 0xff, 0xff, 0xff, 0xee, 0xcc, 0x00, 0x00,
	0xff, 0xff, 0xff, 0xff, 0xff, 0xee, 0xcc, 0x00,
	0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xee, 0xcc,
	0xee, 0xff, 0xff, 0xff, 0xcc, 0x00, 0x00, 0x00,
	0xcc, 0x00, 0x00, 0xff, 0xee, 0xcc, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0xff, 0xee, 0x00, 0x00,
};*/
static uint8_t cursor[CURSOR_WIDTH * CURSOR_HEIGHT] = {
	...
	0xff, 0x00, 0x00, 0x76, 0xff, 0x34, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x1f, 0xff, 0x83, 0x00, 0x00,
};

/* flip pixel */
static void flippixel(ecio_fbinfo_t *fbinfo, int x, int y, uint8_t v) {
	...
	if (old) {
		...
		for (int y = 0; y < CURSOR_HEIGHT; y++) {
			for (int x = 0; x < CURSOR_WIDTH; x++)
				flippixel(fbinfo, ox+x, oy+y, cursor[y * CURSOR_WIDTH + x]);
		}
	}
	if (new) {
		...
		for (int y = 0; y < CURSOR_HEIGHT; y++) {
			for (int x = 0; x < CURSOR_WIDTH; x++)
				flippixel(fbinfo, mx+x, my+y, cursor[y * CURSOR_WIDTH + x]);
		}
	}
}
...
static int run(void) {
	...
	while (running) {
		...
		while (!ec_ioctl(mfd, ECIO_INP_GETEVENT, (uintptr_t)&event)) {
			...
			if (event.state[ECB_RIGHT]) running = false;
		}
